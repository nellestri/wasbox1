...
class UnclaimedLaundrySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // =====================================================================
        // 1. CREATE UNCLAIMED ORDERS (Various urgency levels)
        // =====================================================================
        $this->command->info('Creating unclaimed orders across branches...');
        ...
        foreach ($unclaimedScenarios as $scenario) {
            for ($i = 0; $i < $scenario['count']; $i++) {
                ...
                $order = Order::create([
                    ...
                    'price_per_kg' => $service?->price_per_kg ?? 35,
                    ...
                    'ready_at' => $readyAt,
                    'notes' => "Test unclaimed order - {$scenario['label']}",
                    'created_at' => $receivedAt,
                    'updated_at' => $readyAt,
                ]);

                // Calculate totals
                $subtotal = $order->weight * $order->price_per_kg;
                $order->update([
                    'subtotal' => $subtotal,
                ...
                // Update order with unclaimed tracking fields (if they exist)
                $order->update([
        ...
        $this->command->info("✅ Created {$totalCreated} unclaimed orders");
        ...
        // =====================================================================
        // 4. LINK EXISTING READY ORDERS TO UNCLAIMED TRACKING
        // =====================================================================
        $this->command->info('Linking existing ready orders to unclaimed tracking...');

        $existingReadyOrders = Order::where('status', 'ready')
        ...
        foreach ($existingReadyOrders as $order) {
            $daysUnclaimed = $order->ready_at ? now()->diffInDays($order->ready_at) : 0;
            ...
            if ($daysUnclaimed >= 3) {
                UnclaimedLaundry::create([
                    ...
                    'status' => 'unclaimed',
                    'notes' => "Auto-linked from existing ready order",
        ...
        $this->command->info("✅ Linked {$linkedCount} existing ready orders");
