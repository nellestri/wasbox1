    ...
    public function store(Request $request)
{
    ...
    // Create order (NO pickup_request_id for staff walk-in orders)
    $order = Order::create([
        ...
        // ❌ REMOVED: 'pickup_request_id' => null, // Staff orders don't have pickup requests
    ]);
...
private function generateTrackingNumber(): string
{
    ...
    public function show($id)
    {
        ...
        $pickup = PickupRequest::with(['customer', 'branch', 'service', 'assignedStaff', 'order'])
            ->where('branch_id', $staff->branch_id)
            ->where('id', $id)
            ->firstOrFail();
    ...
    public function markPickedUp($id)
    {
        ...
        return redirect()->route('staff.orders.create', ['pickup_id' => $pickup->id])
            ->with('success', 'Laundry picked up! Now create the order.');
    }
    ...
    public function stats()
    {
        ...
        $stats = [
            'pending' => PickupRequest::where('branch_id', $staff->branch_id)->pending()->count(),
            'accepted' => PickupRequest::where('branch_id', $staff->branch_id)->accepted()->count(),
            'en_route' => PickupRequest::where('branch_id', $staff->branch_id)->enRoute()->count(),
            'picked_up' => PickupRequest::where('branch_id', $staff->branch_id)->pickedUp()->count(),
            'today' => PickupRequest::where('branch_id', $staff->branch_id)->whereDate('preferred_date', today())->count(),
            'my_assigned' => PickupRequest::where('branch_id', $staff->branch_id)->where('assigned_to', $staff->id)->count(),
        ];
