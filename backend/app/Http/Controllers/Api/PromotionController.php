...
class PromotionController extends Controller
{
    ...
    public function validateCode(Request $request)
    {
        ...
        $isApplicable = $promotion->isApplicableTo($orderCandidate);
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'message' => $isApplicable ? 'Promo is applicable' : 'Promo is not applicable to this order',
    ...
    /**
     * Return all promotions applicable to a candidate order (subtotal, weight, service, branch)
    ...
    public function applicable(Request $request)
    {
        ...
        $formatted = $promotions->map(function ($promo) use ($orderCandidate) {
            $isApplicable = $promo->isApplicableTo($orderCandidate);

            $discountValue = 0;
            $finalTotal = $orderCandidate->subtotal;
            $extra = [];

            if ($isApplicable) {
                if ($promo->application_type === 'per_load_override' && isset($orderCandidate->weight)) {
                    $info = $promo->computeOverrideTotal($orderCandidate->weight);
                    $overrideTotal = $info['override_total'];
                    $discountValue = max(0, $orderCandidate->subtotal - $overrideTotal);
                    $finalTotal = $overrideTotal;
                    $extra = [
                        'loads' => $info['loads'],
                        'override_total' => round($overrideTotal, 2),
                    ...
                    $discountValue = min($promo->calculateDiscountValue($orderCandidate->subtotal, $orderCandidate->weight), $orderCandidate->subtotal);
                    $finalTotal = max(0, $orderCandidate->subtotal - $discountValue);
                }
            }
