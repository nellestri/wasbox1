    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $pickup = PickupRequest::where('customer_id', $customer->id)
                ->where('id', $id)
                ->with(['branch', 'service', 'assignedStaff', 'order'])
                ->first();
...
public function store(Request $request)
{
    ...
    /**
     * Link pickup to order (after creating order from pickup)
     *
     * PUT /api/v1/staff/pickups/{id}/link-order
     */
    public function linkOrder(Request $request, $id)
    {
        try {
            $request->validate([
            ...
            return response()->json([
                'success' => true,
                'message' => 'Order linked to pickup request successfully',
            ]);
        } catch (\Exception $e) {
            Log::error('Error linking order to pickup: ' . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Failed to link order',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
