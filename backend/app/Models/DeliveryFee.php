...
class DeliveryFee extends Model
{
     ...
     */
    public function calculateFee(string $serviceType, ?float $orderAmount = null): array
    {
        // Check if order qualifies for free delivery
        if ($orderAmount && $this->minimum_order_for_free && $orderAmount >= $this->minimum_order_for_free) {
            return [
                'pickup_fee' => 0,
                'delivery_fee' => 0,
                'total_fee' => 0,
                'discount_applied' => 'Free delivery (minimum order reached)',
            ];
        }
