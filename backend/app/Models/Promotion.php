...
class Promotion extends Model
{
    ...
    public function scopePerLoadOverride($query)
    {
    ...
    /**
     * Check if promotion is applicable to order data
     */
    public function isApplicableTo(array $orderData): bool
    {
        if (!$this->is_valid) {
            return false;
        }
        ...
        // Check branch
        if ($this->branch_id && $this->branch_id != ($orderData['branch_id'] ?? null)) {
            return false;
        }

        // Check minimum amount
        if ($this->min_amount > 0 && ($orderData['subtotal'] ?? 0) < $this->min_amount) {
            return false;
        }

        // Check applicable services
        if ($this->applicable_services && isset($orderData['service_id'])) {
            if (!in_array($orderData['service_id'], $this->applicable_services)) {
                return false;
            }
        }
    ...
    /**
     * Calculate the effect of this promotion on an order
     */
