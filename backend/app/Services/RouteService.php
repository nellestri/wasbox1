...
class RouteService
{
    ...
    public function getOptimizedRoute($branchId, array $pickupIds, $provider = 'osrm')
    {
        try {
            ...
            $optimizedOrder = $this->solveTSP($matrix['durations']);
            ...
            for ($i = 0; $i < count($optimizedOrder) - 1; $i++) {
                ...
                if ($i > 0 && $i < count($optimizedOrder) - 2) {
            ...
            $detailedRoute = $this->getDetailedRouteForWaypoints($coordinates, $optimizedOrder, $provider);
    ...
    /**
     * Get detailed route for waypoints in optimized order
     */
    private function getDetailedRouteForWaypoints($coordinates, $order, $provider)
    {
        // Build coordinate string in optimized order
        $orderedCoordinates = [];
        foreach ($order as $index) {
            $orderedCoordinates[] = $coordinates[$index];
        }
